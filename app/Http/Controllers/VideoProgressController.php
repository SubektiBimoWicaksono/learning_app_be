    ...
    public function markVideoAsWatched(Request $request)
{
    ...
    // // Fungsi dipanggil saat user klik video
    // public function markVideoAsWatched(Request $request)
    // {
    //     $request->validate([
    //         'video_id' => 'required|exists:videos,id',
    ...
    //     // Ambil course_id dari video
    //     $video = Video::with('section')->findOrFail($videoId);
    //     $courseId = $video->section->course_id;

    //     // Cek apakah semua video sudah ditonton
    //     $totalVideos = Video::whereHas('section', function ($query) use ($courseId) {
    //         $query->where('course_id', $courseId);
    //     })->count();

    //     $watchedVideos = VideoProgress::where('user_id', $userId)
    //         ->whereIn('video_id', function ($query) use ($courseId) {
    //             $query->select('id')
    //                 ->from('videos')
    //                 ->whereIn('section_id', function ($q) use ($courseId) {
    //                     $q->select('id')
    //                         ->from('sections')
    //                         ->where('course_id', $courseId);
    //                 });
    ...
    //     return response()->json([
    //         'message' => 'Video marked as watched',
    //         'completed' => $watchedVideos == $totalVideos
    //     ]);
    // }
}
